...
export default function Quantity_Sale({ next, back, errors = {} }) {
  ...
  console.log("This is error on this page:", errors);

  console.log("This is unit", crop.unit);
  ...
  useEffect(() => {
    ...
    const   unit = units?.find(async (val) => {
       await val?.id === crop?.unitId;
      return await val?.name;
    });
    console.log("this is crop name inside first", unit);
 ...
 const currentTarget = name;
    ...
    console.log("This is selected unit", selectedUnits);
    if (name === "unitId") {
      setselectedUnits(value?.name);
    }
    dispatch(
      setCropData({
        [name]: value?.id,
 ...
 const currentTarget = e.target.name;
    ...
    if (
      ...
      e.target.name == "minimumBidIncrement"
    ) {
      if (val === "" || /^\d+$/.test(val)) {
        dispatch(setCropData({ [e.target.name]: e.target.value }));
      } else {
        let target = e.target.name;
        errors[target] = "Only digits allowed";
        console.log("this is erros", errors);
      }
    } else {
      dispatch(setCropData({ [e.target.name]: e.target.value }));
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col">
              ...
              <CommonDropdown
                ...
                options={units}
                // value={selectedUnits}
                value={units?.find((g) => g?.id === crop?.unitId) || null}
                placeholder="Unit"
                name="unitId"
          ...
          <div className="text-black mt-6">
            ...
            {saleType === "AUCTION" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
                <div className="flex flex-col">
                  <label>{`Price Per ${(selectedUnits && selectedUnits) || "Unit"}`}</label>
                  {/* <input
                  type="text"
                  name="pricePerKg"
                  className="border border-gray-300 mt-1 w-full rounded-lg h-11 px-4"
                  placeholder={`Enter Price Per ${selectedUnits.name || "Unit"}`}
                ...
                /> */}
                  <div className="mt-1">
                    <TextField
                      // type="text"
                      placeholder={`Enter Price Per ${(selectedUnits && selectedUnits) || "Unit"}`}
                      name="pricePerKg"
                      className="w-full text-black  rounded-lg h-11  focus:border-lime-500 outline-none"
                      onChange={handleChange}
                      value={crop.pricePerKg}
                    />
                  </div>
              ...
              <div className="mt-5 flex flex-col">
                ...
                <div className="mt-1">
                  <TextField
                    ...
                    placeholder={`Enter Price Per ${(selectedUnits && selectedUnits) || "Unit"}`}
                    name="pricePerKg"
                    className="w-full text-black border border-gray-300  rounded-lg h-11  focus:border-lime-500 outline-none"
                    onChange={handleChange}
                    value={crop.pricePerKg}
                  />
                  {errors.pricePerKg && (
