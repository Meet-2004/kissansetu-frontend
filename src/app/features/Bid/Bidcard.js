...
export default function CropCard({
  ...
  console.log(buyerActiveTab);

  const dispatch = useDispatch();
  const { auctions, loading, error, totalPages, page } = useSelector(
    (state) => state.bid,
  );

  console.log("this is auctions data", auctions);
  console.log("This is search list at bid card !!!!!!!!!!!!!!!!!!!!",searchList);
  ...
  useEffect(() => {
    ...
    dispatch(fetchAuctions(pageRef.current));
       ...
       dispatch(fetchAuctions({page:0,search:searchList}));
    }
    const interval = setInterval(() => {
      dispatch(fetchAuctions({page:pageRef.current,search:searchList})); // always latest page
    }, 30000);
  ...
  const refreshAuctions = () => {
    if (buyerActiveTab === "Auctions") {
      dispatch(invalidateAuctions());
      dispatch(fetchAuctions({page:0,search:searchList}));
    } else {
      dispatch(invalidatefixed());
      dispatch(fetchFixed({page:pageRef.current,search:searchList}));
    }
  };
  ...
  const handleNextPage = () => {
    ...
    if (page < totalPages - 1) {
      dispatch(fetchAuctions({page:page + 1,search:searchList}));
  ...
  return (
    <section
      className={`mx-auto w-full max-w-7xl ${bidPlaced ? "fixed" : ""} mb-5`}
    >
      {/* HEADER */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <p
            className={`text-black text-xl sm:text-2xl md:text-3xl ${headFont.className}`}
          >
            Marketplace Grid
          </p>
        </div>
        <div className="bg-white p-1 rounded-xl shadow-md flex gap-1 w-fit justify-center items-center">
          <button
            className={`px-4 md: py-2 md: font-medium rounded-lg ${
              buyerActiveTab === "Auctions"
                ? "bg-[#64B900] text-white"
                : "text-black bg-white"
            }`}
            onClick={() => setBuyerActiveTab("Auctions")}
          >
            Auctions
          </button>
          ...
          <button
            className={`px-4 md: py-2 md: font-medium rounded-lg ${
              buyerActiveTab === "Fixed"
                ? "bg-[#64B900] text-white"
                : "text-black bg-white"
            }`}
            onClick={() => setBuyerActiveTab("Fixed")}
          >
            Fixed
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
        {loading && buyerActiveTab === "Auctions" ? (
          <SkeletonCard count={3} />
        ) : error && buyerActiveTab === "Auctions" ? (
              ...
              <div className="p-5">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-gray-500 text-sm">{item.variety}</p>
                  </div>

                  <div className="text-right">
                    <div className="flex gap-1 items-center">
                      <p className="text-gray-800 text-sm font-medium">dnd</p>
      ...
      {/* FIXED TAB */}
      {buyerActiveTab === "Fixed" && <FixedCards buyerActiveTab={buyerActiveTab} searchList={searchList}  />}

      {/* PAGINATION */}
      {buyerActiveTab === "Auctions" && auctions?.totalPages > 0 && (
      ...
      {openRequirement && (
        <RequirementModal closeRequirement={closeRequirement} />
      )}
    </section>
  );
}
