...
import { putMultipartApi, deleteApi } from "@/services/apiService";
import { toast } from "react-toastify";
import Toast from "@/Components/Alert/Toast";
import ExtendAuctionModal from "../Extend Auction Time/ExtendAuctionTime";

export default function CropViewModal({
  crop,
  onClose,
  selectedCrop,
  setrefreshKey,
  ...
  const deleteListing = async (listingId) => {
    const deleteListing = await deleteApi(`/listings/${listingId}`);
    console.log("this is deletelisting", deleteListing);

    if (deleteListing.success) {
      setrefreshKey();
      toast.success(deleteListing.message);
      setTimeout(() => {
        onClose();
      }, 2000);
    }else{
       toast.error(deleteListing.message);
    }
  };
  ...
  useEffect(() => {
    ...
    fetchListData();
    console.log("hello");
  }, [crop,refreshExtendAuctionTimeCount]);
  ...
  return (
    <>
      ...
      <div className="fixed  z-50 inset-0 bg-black/20  flex justify-center items-center p-4">
              ...
              >
                ...
                <p>💬 {crop?.messages} Msg</p>
                    ...
                    <div className="bg-gray-50 p-5 rounded-xl h-fit shadow-lg border-2 border-gray-300">
                      ...
                      {crop?.saleType === "AUCTION" && (
                        <button className="w-full border py-2 rounded-lg mb-3 text_black cursor-pointer" onClick={()=>{setExtendAuctionTime(true)}}>
                          ⏱ Extend Auction Time
                        </button>
                      )}
                        ...
                        <>
                          ...
                          <div className="grid grid-cols-2 gap-5 mt-3">
                            ...
                            <div>
                              ...
                              <p className="text_black text-sm">
                                {crop?.auctionEndTime[2]}-
                                {crop?.auctionEndTime[1]}-
                            ...
                            return (
                              <div className="flex items-center justify-center p-5">
                                <div className="text-black text-xl">
                                  There are no bids currently on this listing
                                  yet
                                </div>
                              </div>
                            );
                                      ...
                                      >
                                        ...
                                        <td className="px-4 py-3">
                                          <div className="flex flex-wrap gap-2">
                                            ...
                                            <button
                                              className="rounded-lg border text_lime px-3 py-1 text-sm font-semibold  hover:back_lime hover:text-white cursor-pointer"
                                              onClick={() =>
                                                handleMessageClick(bid.buyerId)
                                              }
                                            >
                                              Message
                                            </button>
        ...
        {
          extendAuctionTime && <ExtendAuctionModal onClose={()=>setExtendAuctionTime(false)} setRefreshExtendAuctionTimeCount={()=>setRefreshExtendAuctionTimeCount(refreshExtendAuctionTimeCount+1)}/>
        }
      </div>
    </>
  );
