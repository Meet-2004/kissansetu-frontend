...
import Sell from "@/SVG/Dashboard/Sell.svg";
import Buyer from "@/SVG/Dashboard/Buyer.svg";
...
import Sell_black from "@/SVG/Dashboard/My_Listing.svg";
import { useState } from "react";
import { BorderColor, Style } from "@mui/icons-material";
...
import ListingCard from "@/app/features/cropListings/ListingCard";
  ...
  <div className="space-y-6 mt-5 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
    ...
    <div className="flex justify-center">
      ...
      <div className="bg-white p-1 sm:p-2 rounded-xl shadow-md flex gap-1 sm:gap-2 flex-wrap">
        ...
        {/* SELL BUTTON */}
        <button
          className={`px-4 sm:px-6 lg:px-8 py-2 sm:py-3 font-medium flex items-center gap-2 rounded-lg cursor-pointer transition
          ${activeTab === "SELL"
    ...
    {activeTab === "BUYER" && (
      ...
      <RequirementBanner onClick={()=>setopenRequirement(true)} />
        <MarketInsights />
        <Bidcard openRequirement={openRequirement} closeRequirement={()=>setopenRequirement(false)}/>
        </div>
      </>
    )}
      {/* STYLE */}
