...
export default function Navbar_Dashboard({
  ...
  const authDetailsParsed = JSON?.parse(authDetails) || {user:"Your Name",email:"Your Email"};
  const userName = JSON?.parse(authDetailsParsed?.user) || {user:"Your Name",email:"Your Email"};
  const userMail = JSON?.parse(authDetailsParsed?.userMail) || {user:"Your Name",email:"Your Email"};
  ...
  return (
    <header className="bg-white border-b px-4 sm:px-6 h-16 flex items-center justify-between relative shrink-0">
      ...
      <div className="flex items-center gap-3">
        ...
        >
          <Image src={toogle} alt="menu" width={22} height={22} />
        </button>

        {/* TITLE */}
        <h1 className=" text-black sm:text-lg">
          {/* Mobile */}
          <span className="block md:hidden">KissanSetu</span>
      ...
      <div className="flex items-center gap-3 sm:gap-5">
        ...
        >
          ...
          <div className="hidden sm:block text-right">
            ...
            <p className="text-gray-500 text-xs">{userMail}</p>
          </div> 
         ...
         <div className="h-9 w-9 sm:h-10 sm:w-10 rounded-full bg-lime-500 flex items-center justify-center text-white font-bold">
          ...
          {/* DROPDOWN */}
          {profileDown && (
            <div className="absolute right-0 top-12 w-44 bg-white border rounded-xl shadow-xl overflow-hidden mt-3 z-50">
              <button
              ...
              >
                ...
                <span className="text-gray-700 text-sm">View Profile</span>
              </Link>
            </div>
          )}
          {notificationDropDown && <NotificationPopUp />}
        </div>
      </div>
    </header>
  );
